from flask import Flask, request, jsonify
from flask_cors import CORS
import os
import pefile
import numpy as np
import tensorflow as tf
import shutil

app = Flask(__name__)
CORS(app)

# Load the model when starting the app (do not load it on every request)
model = tf.keras.models.load_model("best_model_v4.keras")

dlls_list = [
    "advapi32.dll",
    "kernel32.dll",
    "vspmsg.dll",
    "ole32.dll",
    "oleaut32.dll",
    "psapi.dll",
    "setupapi.dll",
    "shlwapi.dll",
    "pdh.dll",
    "xmllite.dll",
    "msvcr110.dll",
    "user32.dll",
    "msvcrt.dll",
    "shell32.dll",
    "ntdll.dll",
    "api-ms-win-core-winrt-l1-1-0.dll",
    "dui70.dll",
    "windows.ui.immersive.dll",
    "msvcr100.dll",
    "atl100.dll",
    "msvcp100.dll",
    "version.dll",
    "mspdbcore.dll",
    "rpcrt4.dll",
    "secur32.dll",
    "userenv.dll",
    "mpclient.dll",
    "cabinet.dll",
    "comctl32.dll",
    "gdi32.dll",
    "api-ms-win-core-com-l1-1-1.dll",
    "api-ms-win-core-synch-l1-2-0.dll",
    "api-ms-win-core-processthreads-l1-1-2.dll",
    "api-ms-win-core-errorhandling-l1-1-1.dll",
    "api-ms-win-core-libraryloader-l1-2-0.dll",
    "api-ms-win-core-profile-l1-1-0.dll",
    "api-ms-win-core-sysinfo-l1-2-1.dll",
    "api-ms-win-core-string-l1-1-0.dll",
    "api-ms-win-core-registry-l1-1-0.dll",
    "api-ms-win-core-io-l1-1-1.dll",
    "api-ms-win-core-path-l1-1-0.dll",
    "api-ms-win-eventing-provider-l1-1-0.dll",
    "api-ms-win-core-string-l2-1-0.dll",
    "api-ms-win-eventing-controller-l1-1-0.dll",
    "api-ms-win-core-file-l1-2-1.dll",
    "sqlos.dll",
    "netapi32.dll",
    "opends60.dll",
    "sqlmin.dll",
    "sqllang.dll",
    "sqltses.dll",
    "sqldk.dll",
    "api-ms-win-core-rtlsupport-l1-2-0.dll",
    "api-ms-win-core-heap-l2-1-0.dll",
    "api-ms-win-core-heap-l1-2-0.dll",
    "api-ms-win-core-util-l1-1-0.dll",
    "api-ms-win-core-memory-l1-1-2.dll",
    "api-ms-win-core-interlocked-l1-2-0.dll",
    "api-ms-win-core-debug-l1-1-1.dll",
    "api-ms-win-core-handle-l1-1-0.dll",
    "winmm.dll",
    "mfc100u.dll",
    "clusapi.dll",
    "resutils.dll",
    "mpr.dll",
    "crypt32.dll",
    "dbghelp.dll",
    "odbc32.dll",
    "odbcbcp.dll",
    "ws2_32.dll",
    "authz.dll",
    "sspicli.dll",
    "wkscli.dll",
    "netutils.dll",
    "comdlg32.dll",
    "mfc110u.dll",
    "msvcp110.dll",
    "spyxxhk.dll",
    "wtsapi32.dll",
    "instapi110.dll",
    "samcli.dll",
    "nislog.dll",
    "wininet.dll",
    "srvcli.dll",
    "msvcp110_win.dll",
    "api-ms-win-shcore-scaling-l1-1-1.dll",
    "atl110.dll",
    "gdiplus.dll",
    "sqlresourceloader.dll",
    "api-ms-win-core-winrt-string-l1-1-0.dll",
    "spyxxhk_amd64.dll",
    "iphlpapi.dll",
    "msvcr120_clr0400.dll",
    "webengine4.dll",
    "cryptui.dll",
    "dtuparse.dll",
    "mspdb110.dll",
    "sqlncli11.dll",
    "mfc42u.dll",
    "atl.dll",
    "powrprof.dll",
    "framedynos.dll",
    "wsock32.dll",
    "propsys.dll",
    "api-ms-win-core-apiquery-l1-1-0.dll",
    "api-ms-win-core-shlwapi-legacy-l1-1-0.dll",
    "iertutil.dll",
    "uxtheme.dll",
    "windowscodecs.dll",
    "spp.dll",
    "srclient.dll",
    "srcore.dll",
    "vssapi.dll",
    "logoncli.dll",
    "api-ms-win-core-localization-l1-2-1.dll",
    "api-ms-win-core-console-l1-1-0.dll",
    "api-ms-win-core-processenvironment-l1-2-0.dll",
    "cmutil.dll",
    "rasapi32.dll",
    "winhttp.dll",
    "msctfmonitor.dll",
    "slc.dll",
    "msvcr90.dll",
    "onmain.dll",
    "ntdsapi.dll",
    "bcrypt.dll",
    "api-ms-win-security-base-l1-2-0.dll",
    "api-ms-win-security-lsapolicy-l1-1-0.dll",
    "samlib.dll",
    "cfgmgr32.dll",
    "scansetting.dll",
    "rtutils.dll",
    "rtinfo.dll",
    "iisexpresshelper.dll",
    "iisutil2.dll",
    "nativrd2.dll",
    "api-ms-win-core-winrt-error-l1-1-0.dll",
    "msi.dll",
    "ncrypt.dll",
    "urlmon.dll",
    "wintrust.dll",
    "dteparse.dll",
    "dtepkg.dll",
    "sqltdiagn.dll",
    "datacollectorcontroller.dll",
    "wldap32.dll",
    "winsta.dll",
    "mapi32.dll",
    "api-ms-win-core-winrt-error-l1-1-1.dll",
    "winspool.drv",
    "api-ms-win-core-crt-l1-1-0.dll",
    "api-ms-win-core-crt-l2-1-0.dll",
    "sspisrv.dll",
    "api-ms-win-core-heap-obsolete-l1-1-0.dll",
    "api-ms-win-core-processenvironment-l1-1-0.dll",
    "api-ms-win-core-processthreads-l1-1-0.dll",
    "api-ms-win-core-synch-l1-1-0.dll",
    "api-ms-win-security-base-l1-1-0.dll",
    "api-ms-win-core-threadpool-l1-2-0.dll",
    "api-ms-win-core-sysinfo-l1-1-0.dll",
    "api-ms-win-core-windowserrorreporting-l1-1-0.dll",
    "api-ms-win-service-winsvc-l1-2-0.dll",
    "api-ms-win-service-core-l1-1-1.dll",
    "api-ms-win-core-sidebyside-l1-1-0.dll",
    "api-ms-win-core-threadpool-private-l1-1-0.dll",
    "api-ms-win-core-delayload-l1-1-1.dll",
    "appvisvsubsystems32.dll",
    "vcruntime140.dll",
    "api-ms-win-crt-time-l1-1-0.dll",
    "api-ms-win-crt-runtime-l1-1-0.dll",
    "api-ms-win-crt-filesystem-l1-1-0.dll",
    "api-ms-win-crt-stdio-l1-1-0.dll",
    "api-ms-win-crt-string-l1-1-0.dll",
    "api-ms-win-crt-convert-l1-1-0.dll",
    "api-ms-win-crt-heap-l1-1-0.dll",
    "api-ms-win-crt-math-l1-1-0.dll",
    "api-ms-win-crt-locale-l1-1-0.dll",
    "api-ms-win-crt-utility-l1-1-0.dll",
    "msimg32.dll",
    "msvcp140.dll",
    "appvisvsubsystems64.dll",
    "boost_date_time-vc120-mt-1_56.dll",
    "boost_filesystem-vc120-mt-1_56.dll",
    "boost_iostreams-vc120-mt-1_56.dll",
    "boost_program_options-vc120-mt-1_56.dll",
    "boost_system-vc120-mt-1_56.dll",
    "msvcp120.dll",
    "msvcr120.dll",
    "cefcommon.dll",
    "libcef.dll",
    "opengl32.dll",
    "pocofoundation64.dll",
    "libprotobuf.dll",
    "chrome_elf.dll",
    "sdl.dll",
    "xinput1_3.dll",
    "dinput8.dll",
    "pthreadvse2.dll",
    "lz32.dll",
    "crashreport.dll",
    "engine.dll",
    "mfc140.dll",
    "crlutl.dll",
    "crli18n.dll",
    "crlctl.dll",
    "crlutils.dll",
    "mfc120u.dll",
    "crlcomponent.dll",
    "d3dcompiler_42.dll",
    "d3dx9_42.dll",
    "d3dx11_42.dll",
    "mscoree.dll",
    "freeimage.dll",
    "api-ms-win-shcore-obsolete-l1-1-0.dll",
    "api-ms-win-core-string-obsolete-l1-1-0.dll",
    "api-ms-win-core-kernel32-legacy-l1-1-1.dll",
    "imm32.dll",
    "ninput.dll",
    "api-ms-win-core-registry-l1-1-1.dll",
    "msocf.dll",
    "qt5printsupport.dll",
    "qt5widgets.dll",
    "qt5gui.dll",
    "qt5core.dll",
    "hha.dll",
    "imagehlp.dll",
    "jail_container.dll",
    "libmwms.dll",
    "libmwservices.dll",
    "java_launcher.dll",
    "oledlg.dll",
    "olepro32.dll",
    "oleacc.dll",
    "cdlmso.dll",
    "devobj.dll",
    "libreadline6.dll",
    "sqlite.dll",
    "avcodec-56.dll",
    "avdevice-56.dll",
    "avfilter-5.dll",
    "avformat-56.dll",
    "avresample-2.dll",
    "avutil-54.dll",
    "postproc-53.dll",
    "swresample-1.dll",
    "swscale-3.dll",
    "pgodb110.dll",
    "wevtapi.dll",
    "sti.dll",
    "msvcp90.dll",
    "libz.dll",
    "sysdm.cpl",
    "crtdll.dll",
    "libgimp-2.0-0.dll",
    "libgimpbase-2.0-0.dll",
    "libgimpcolor-2.0-0.dll",
    "libexif-12.dll",
    "libglib-2.0-0.dll",
    "libintl-8.dll",
    "libjpeg-8.dll",
    "olmapi32.dll",
    "scnpst32.dll",
    "scnpst64.dll",
    "sfbappsdk.dll",
    "shcore.dll",
    "fd.dll",
    "oisapp.dll",
    "newdev.dll",
    "commonlib.dll",
    "pluginkernel.dll",
    "filesystem.dll",
    "versionmodule.dll",
    "difxapi.dll",
    "libgcc_s_seh-1.dll",
    "libstdc++-6.dll",
    "sqlite3.dll",
    "fileloader.dll",
    "dibmodule.dll",
    "ggspawn.dll",
    "ggdownloader.dll",
    "zip7module.dll",
    "ssleay32.dll",
    "libeay32.dll",
    "libdbus-1-3.dll",
    "libexpat-1.dll",
    "libfontconfig-1.dll",
    "libfreetype-6.dll",
    "libgeos_c-1.dll",
    "libgeotiff-2.dll",
    "libtiff-5.dll",
    "libgettextlib-0-19-5-1.dll",
    "libgimpui-2.0-0.dll",
    "libgimpwidgets-2.0-0.dll",
    "libgobject-2.0-0.dll",
    "libgtk-win32-2.0-0.dll",
    "libgimpmath-2.0-0.dll",
    "libgdk_pixbuf-2.0-0.dll",
    "libcairo-2.dll",
    "libgdk-win32-2.0-0.dll",
    "libpango-1.0-0.dll",
    "libpangocairo-1.0-0.dll",
    "libgimpconfig-2.0-0.dll",
    "libgio-2.0-0.dll",
    "liblcms2-2.dll",
    "libpng16-16.dll",
    "libxpm-nox4.dll",
    "libwmf-0-2-7.dll",
    "libwmflite-0-2-7.dll",
    "zlib1.dll",
    "librsvg-2-2.dll",
    "gsdll64.dll",
    "libwebkitgtk-1.0-0.dll",
    "libmng-1.dll",
    "libgthread-2.0-0.dll",
    "libgmodule-2.0-0.dll",
    "libglpk-40.dll",
    "libgmp-10.dll",
    "libisl-10.dll",
    "libgnutls-30.dll",
    "libhogweed-4.dll",
    "libiconv-2.dll",
    "libnettle-6.dll",
    "libpcreposix-0.dll",
    "libgsl-19.dll",
    "libhdf5_hl-9.dll",
    "libhdf5-9.dll",
    "libhttpd.dll",
    "libaprutil-1.dll",
    "libapr-1.dll",
    "libwinpthread-1.dll",
    "libtermcap.dll",
    "libmwi18n.dll",
    "libnetcdf-7.dll",
    "liboctave-4.dll",
    "liboctinterp-4.dll",
    "liboctgui-2.dll",
    "libpq.dll",
    "libproj-12.dll",
    "libpstoedit-0.dll",
    "libqhull-5.dll",
    "libsybdb-5.dll",
    "libut.dll",
    "uiw.dll",
    "libvlc.dll",
    "libxml2-2.dll",
    "libzmq.dll",
    "mfc90u.dll",
    "msvcr80.dll",
    "msvbvm60.dll",
    "msvcp_win.dll",
    "api-ms-win-crt-private-l1-1-0.dll",
    "ext-ms-win-com-ole32-l1-1-1.dll",
    "ext-ms-win-shell-shell32-l1-2-0.dll",
    "dmcommandlineutils.dll",
    "ppcore.dll",
    "glib-2.0.dll",
    "sigc-2.0.dll",
    "vmapputil.dll",
    "vmclientcore.dll",
    "vmdbcom.dll",
    "vmwarebase.dll",
    "vmwarecui.dll",
    "vmwarestring.dll",
    "vmwarewui.dll",
    "vixdiskmountapi.dll",
    "gobject-2.0.dll",
    "gvmomi.dll",
    "vmappcfg.dll",
    "msvcm90.dll",
    "wlanapi.dll",
    "dwmapi.dll",
    "wbemcomn.dll",
    "api-ms-win-core-registry-l2-2-0.dll",
    "api-ms-win-core-shlwapi-obsolete-l1-2-0.dll",
    "api-ms-win-core-localization-obsolete-l1-3-0.dll",
    "api-ms-win-core-version-l1-1-0.dll",
    "mswsock.dll",
    "api-ms-win-core-file-l2-1-1.dll",
    "api-ms-win-eventing-classicprovider-l1-1-0.dll",
    "api-ms-win-appmodel-runtime-l1-1-1.dll",
    "mfcore.dll",
    "mfplat.dll",
    "api-ms-win-security-sddl-l1-1-0.dll",
    "api-ms-win-core-synch-l1-2-1.dll",
    "api-ms-win-security-provider-l1-1-0.dll",
    "api-ms-win-security-trustee-l1-1-1.dll",
    "coremessaging.dll",
    "coreuicomponents.dll",
    "api-ms-win-security-lsalookup-l2-1-1.dll",
    "api-ms-win-core-timezone-l1-1-0.dll",
    "scecli.dll",
    "api-ms-win-core-psapi-l1-1-0.dll",
    "nsi.dll",
    "snmpapi.dll",
    "api-ms-win-core-console-l2-1-0.dll",
    "api-ms-win-core-wow64-l1-1-0.dll",
    "api-ms-win-core-kernel32-private-l1-1-2.dll",
    "api-ms-win-downlevel-shlwapi-l1-1-1.dll",
    "api-ms-win-downlevel-shlwapi-l2-1-1.dll",
    "loadperf.dll",
    "api-ms-win-base-util-l1-1-0.dll",
    "api-ms-win-core-libraryloader-l1-2-1.dll",
    "api-ms-win-core-datetime-l1-1-1.dll",
    "api-ms-win-service-management-l1-1-0.dll",
    "api-ms-win-service-management-l2-1-0.dll",
    "browcli.dll",
    "certca.dll",
    "certenroll.dll",
    "api-ms-win-eventing-consumer-l1-1-0.dll",
    "dnsapi.dll",
    "maintenanceui.dll",
    "dmpushproxy.dll",
    "dmcmnutils.dll",
    "dsclient.dll",
    "omadmapi.dll",
    "dmxmlhelputils.dll",
    "dhcpcsvc.dll",
    "dhcpcsvc6.dll",
    "api-ms-win-eventing-legacy-l1-1-0.dll",
    "api-ms-win-core-file-l2-1-2.dll",
    "wsmsvc.dll",
    "unattend.dll",
    "actionqueue.dll",
    "api-ms-win-core-privateprofile-l1-1-1.dll",
    "nodewinrtwrap.dll",
    "api-ms-win-downlevel-kernel32-l2-1-0.dll",
    "api-ms-win-power-setting-l1-1-0.dll",
    "combase.dll",
    "api-ms-win-core-threadpool-legacy-l1-1-0.dll",
    "msys-1.0.dll",
    "msys-intl-8.dll",
    "msys-regex-1.dll",
    "msys-iconv-2.dll",
    "msys-termcap-0.dll",
    "msys-perl5_8.dll",
    "msys-crypto-1.0.0.dll",
    "msys-ssl-1.0.0.dll",
    "msys-magic-1.dll",
    "mvm.dll",
    "oemuninstall.dll",
    "php5ts.dll",
    "python27.dll",
    "qt5dbus.dll",
    "qt5designercomponents.dll",
    "qt5designer.dll",
    "qt5xml.dll",
    "qt5network.dll",
    "qt5quick.dll",
    "qt5qml.dll",
    "avrt.dll",
    "sensapi.dll",
    "statreport.dll",
    "lua51.dll",
    "galaxy.dll",
    "ter32.dll",
    "tier0.dll",
    "vstdlib.dll",
    "qtcore4.dll",
    "qtgui4.dll",
    "steam_api64.dll",
    "nvsmartmax.dll",
    "esscli.dll",
    "nvsmartmax64.dll",
    "sourcecontrol.dll",
    "zlibwapi.dll",
    "wtli.dll",
    "cpfe.dll",
    "jxbrowser-chromium-lib.dll",
    "apphelp.dll",
    "api-ms-win-core-localization-l1-2-2.dll",
    "api-ms-win-core-localization-private-l1-1-0.dll",
    "appxalluserstore.dll",
    "msvcdis110.dll",
    "perl524.dll",
    "libjasper-4.dll",
    "tapi32.dll",
    "cryptsp.dll",
    "wer.dll",
    "api-ms-win-core-file-l1-2-2.dll",
    "nvml.dll",
    "libpoppler-glib-8.dll",
    "api-ms-win-security-audit-l1-1-1.dll",
    "api-ms-win-security-sddlparsecond-l1-1-0.dll",
    "url.dll",
    "avicap32.dll",
    "browseui.dll",
    "ddraw.dll",
    "icmp.dll",
    "ipsecsnp.dll",
    "kbdit.dll",
    "kbdkyr.dll",
    "msdmo.dll",
    "msident.dll",
    "msports.dll",
    "occache.dll",
    "panmap.dll",
    "ulib.dll",
    "winscard.dll",
    "actxprxy.dll",
    "adsnt.dll",
    "avifil32.dll",
    "catsrvps.dll",
    "certcli.dll",
    "certmgr.dll",
    "console.dll",
    "d3dxof.dll",
    "dbnetlib.dll",
    "dbnmpntw.dll",
    "filemgmt.dll",
    "iasacct.dll",
    "iepeers.dll",
    "inetcomm.dll",
    "inseng.dll",
    "kbdal.dll",
    "kbdfi.dll",
    "kbdic.dll",
    "kbdno.dll",
    "midimap.dll",
    "mspatcha.dll",
    "ntmarta.dll",
    "shdocvw.dll",
    "webclnt.dll",
    "mfc42.dll",
    "msvcp60.dll",
    "dwrite.dll",
    "d2d1.dll",
    "mscms.dll",
    "xolehlp.dll",
    "ntlanman.dll",
    "mprapi.dll",
    "dispex.dll",
    "iassdo.dll",
    "icm32.dll",
    "kbdlv1.dll",
    "odbccu32.dll",
    "sfc_os.dll",
    "w32topl.dll",
    "kbdcan.dll",
    "kbdhe.dll",
    "kbdycl.dll",
    "msisip.dll",
    "sqlsrv32.dll",
    "activeds.dll",
    "httpapi.dll",
    "sfc.dll",
    "ktmw32.dll",
    "comsvcs.dll",
    "kbdir.dll",
    "query.dll",
    "winrnr.dll",
    "msvidctl.dll",
    "cdosys.dll",
    "docprop.dll",
    "efsadu.dll",
    "fontsub.dll",
    "icmui.dll",
    "kbdbe.dll",
    "kbdhe319.dll",
    "kbdlt1.dll",
    "kbdusl.dll",
    "msctf.dll",
    "msls31.dll",
    "shfolder.dll",
    "themeui.dll",
    "usp10.dll",
    "wship6.dll",
    "zipfldr.dll",
    "msacm32.dll",
    "webservices.dll",
    "normaliz.dll",
    "wsnmp32.dll",
    "mf.dll",
    "adsldpc.dll",
    "msimtf.dll",
    "oleprn.dll",
    "pstorec.dll",
    "api-ms-win-crt-multibyte-l1-1-0.dll",
    "msvfw32.dll",
    "hhctrl.ocx",
    "glu32.dll",
    "clfsw32.dll",
    "dhcpsapi.dll",
    "iscsidsc.dll",
    "msrating.dll",
    "rpcns4.dll",
    "rtm.dll",
    "wow32.dll",
    "api-ms-win-core-memory-l1-1-0.dll",
    "api-ms-win-core-console-l3-2-0.dll",
    "api-ms-win-core-localization-l1-2-0.dll",
    "api-ms-win-core-errorhandling-l1-1-0.dll",
    "api-ms-win-core-processthreads-l1-1-1.dll",
    "api-ms-win-core-interlocked-l1-1-0.dll",
    "api-ms-win-core-debug-l1-1-0.dll",
    "api-ms-win-core-rtlsupport-l1-1-0.dll",
    "api-ms-win-core-file-l1-1-0.dll",
    "api-ms-win-core-heap-l1-1-0.dll",
    "api-ms-win-ntuser-sysparams-l1-1-0.dll",
    "odbccp32.dll",
    "api-ms-win-crt-environment-l1-1-0.dll",
    "api-ms-win-core-memory-l1-1-3.dll",
    "api-ms-win-core-datetime-l1-1-0.dll",
    "api-ms-win-core-psapi-ansi-l1-1-0.dll",
    "api-ms-win-core-fibers-l1-1-0.dll",
    "api-ms-win-core-file-l2-1-0.dll",
    "api-ms-win-core-sysinfo-l1-2-0.dll",
    "dbgeng.dll"
]

def extract_dll_vector(exe_path, dlls_list):
    try:
        pe = pefile.PE(exe_path)
        dlls_file = {
            entry.dll.decode('utf-8').lower()
            for entry in getattr(pe, 'DIRECTORY_ENTRY_IMPORT', [])
        }
        pe.close()  # Close the file
    except Exception as e:
        print(f"[ERROR] No se pudo abrir el archivo: {e}")
        return None

    return [1 if dll in dlls_file else 0 for dll in dlls_list]

@app.route('/ping', methods=['GET'])
def ping():
    return jsonify({"status": "ok"})

@app.route('/analyze', methods=['POST'])
def analyze():
    file = request.files['file']
    filename = file.filename
    filepath = os.path.join("uploads", filename)

    # Create the 'uploads' folder if it does not exist
    os.makedirs("uploads", exist_ok=True)

    # Save the file
    file.save(filepath)

    try:
        # Extract DLLs vector
        dll_vector = extract_dll_vector(filepath, dlls_list)
        if dll_vector is None:
            return jsonify({"error": "The file could not be analyzed."}), 400

        entry = np.array(dll_vector, dtype=np.float32).reshape(1, len(dlls_list), 1)
        prediction = model.predict(entry)
        predicted_class = np.argmax(prediction)

        classes = ["Benign", "RedLineStealer", "Downloader", "RAT", "BankingTrojan", "SnakeKeyLogger", "Spyware"]
        return jsonify({"filename": filename, "result": classes[predicted_class]})

    finally:
        # Always try to delete the file (if it exists)
        if os.path.exists(filepath):
            try:
                shutil.rmtree('uploads', ignore_errors=True)  # Delete the entire folder and its contents without error if it no longer exists
            except Exception as e:
                print(f"[WARNING] The temporary file could not be deleted: {e}")

if __name__ == '__main__':
    app.run(port=5000, debug=True)
